...
local AppName = "IkanTongkol"
local NotificationDuration = 6.5
local RayfieldFolder = AppName
local ConfigurationFolder = RayfieldFolder.."/Configurations"
local ConfigurationExtension = ".test"
...
function RayfieldLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if Rayfield.Name == AppName or Rayfield.Name == "Rayfield" then
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	-- defaults & layout switch
	Settings = Settings or {}
	Settings.Name = Settings.Name or AppName
	local UseSidebar = (not Settings.Layout) or (string.lower(Settings.Layout) == "sidebar")
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or (AppName.." Interface Suite")
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= (AppName.." Interface Suite") then
		LoadingFrame.Version.Text = AppName.." UI"
	end
	...
	-- ==== SIDEBAR LAYOUT ====
	local function SetupSidebar(margin, sideWidth)
		if not UseSidebar then return end
		margin = margin or 10
		sideWidth = sideWidth or 180

		-- Layout TabList jadi vertikal penuh
		local ll = TabList:FindFirstChildOfClass("UIListLayout") or Instance.new("UIListLayout", TabList)
		ll.FillDirection = Enum.FillDirection.Vertical
		ll.HorizontalAlignment = Enum.HorizontalAlignment.Stretch
		ll.SortOrder = Enum.SortOrder.LayoutOrder
		ll.Padding = UDim.new(0, 6)

		-- gaya tombol tab agar full-width & teks rata kiri
		local function styleTab(frame)
			if frame.ClassName ~= "Frame" or frame.Name == "Template" or frame.Name == "Placeholder" then return end
			frame.Size = UDim2.new(1, 0, 0, 32)
			local img = frame:FindFirstChild("Image")
			local ttl = frame:FindFirstChild("Title")
			if img then
			...
			if ttl then
				...
				ttl.Position = img and UDim2.new(0, 36, 0.5, 0) or UDim2.new(0, 12, 0.5, 0)
			end
		end
		for _, ch in ipairs(TabList:GetChildren()) do styleTab(ch) end
		TabList.ChildAdded:Connect(function(ch) task.defer(function() styleTab(ch) end) end)

		-- posisikan sidebar kiri & konten kanan
		local function apply()
			local topH = Topbar.AbsoluteSize.Y > 0 and Topbar.AbsoluteSize.Y or 45
			TabList.Position  = UDim2.new(0, margin, 0, topH + margin)
			TabList.Size      = UDim2.new(0, sideWidth, 1, -(topH + margin*2))
			Elements.Position = UDim2.new(0, sideWidth + margin*2, 0, topH + margin)
			Elements.Size     = UDim2.new(1, -(sideWidth + margin*3), 1, -(topH + margin*2))
		end

		apply()
		Main:GetPropertyChangedSignal("Size"):Connect(apply)
		Topbar:GetPropertyChangedSignal("Size"):Connect(apply)
	end
